...
import datetime
import re
from cleancat import (Bool, Choices, DateTime, Email, Embedded, Integer, List,
                      Regex, Schema, String, URL)
...
class FieldTestCase(ValidationTestCase):
    def test_string(self):
        ...
        self.assertValid(TextSchema({'text': 'hello world'}),
                         {'text': 'hello world'})
        self.assertInvalid(TextSchema({'text': ''}),
                           {'field-errors': ['text']})
        self.assertInvalid(TextSchema({'text': None}),
                           {'field-errors': ['text']})
        self.assertInvalid(TextSchema({}), {'field-errors': ['text']})
        self.assertInvalid(TextSchema({'text': True}),
                           {'field-errors': ['text']})

        self.assertValid(OptionalTextSchema({'text': 'hello world'}),
                         {'text': 'hello world'})
    ...
    def test_bool(self):
        ...
        self.assertInvalid(FlagSchema({'flag': None}),
                           {'field-errors': ['flag']})
        self.assertInvalid(FlagSchema({}), {'field-errors': ['flag']})
        self.assertInvalid(FlagSchema({'flag': ''}),
                           {'field-errors': ['flag']})
        ...
        self.assertInvalid(OptionalFlagSchema({'flag': ''}),
                           {'field-errors': ['flag']})
    ...
    def test_regex(self):
        ...
        self.assertInvalid(RegexSchema({'letter': 'A'}),
                           {'field-errors': ['letter']})
        self.assertInvalid(RegexSchema({'letter': ''}),
                           {'field-errors': ['letter']})
        self.assertInvalid(RegexSchema({'letter': 'aa'}),
                           {'field-errors': ['letter']})

        self.assertValid(RegexOptionsSchema({'letter': 'a'}), {'letter': 'a'})
        self.assertValid(RegexOptionsSchema({'letter': 'A'}), {'letter': 'A'})
        self.assertInvalid(RegexOptionsSchema({'letter': ''}),
                           {'field-errors': ['letter']})
        self.assertInvalid(RegexOptionsSchema({'letter': 'aa'}),
                           {'field-errors': ['letter']})
        ...
        self.assertEqual(schema.field_errors['letter'],
        ...
        self.assertValid(OptionalRegexMessageSchema({'letter': 'a'}),
                         {'letter': 'a'})
        self.assertValid(OptionalRegexMessageSchema({'letter': ''}),
                         {'letter': ''})
    ...
    def test_datetime(self):
        ...
        self.assertValid(DateTimeSchema({'dt': '2012-10-09'}),
                         {'dt': datetime.date(2012, 10, 9)})
        self.assertValid(DateTimeSchema({'dt': '2012-10-09 13:10:04'}),
                         {'dt': datetime.datetime(2012, 10, 9, 13, 10, 4)})
        self.assertValid(DateTimeSchema({'dt': '2013-03-27T01:24:50.137000+00:00'}),
                         {'dt': datetime.datetime(2013, 3, 27, 1, 24, 50, 137000, tzinfo=utc)})
        self.assertInvalid(DateTimeSchema({'dt': '0000-01-01T00:00:00-08:00'}),
                           {'field-errors': ['dt']})
        self.assertInvalid(DateTimeSchema({'dt': '2012a'}),
                           {'field-errors': ['dt']})
        self.assertInvalid(DateTimeSchema({'dt': ''}),
                           {'field-errors': ['dt']})
        self.assertInvalid(DateTimeSchema({'dt': None}),
                           {'field-errors': ['dt']})
    ...
    def test_email(self):
        ...
        self.assertValid(EmailSchema({'email': 'test@example.com'}),
                         {'email': 'test@example.com'})
        schema = EmailSchema({'email': 'test@example'})
        self.assertInvalid(schema, {'field-errors': ['email']})
        self.assertEqual(schema.field_errors['email'],
                         'Invalid email address.')
        self.assertInvalid(EmailSchema({'email': 'test.example.com'}),
                           {'field-errors': ['email']})
        self.assertInvalid(EmailSchema({'email': None}),
                           {'field-errors': ['email']})
        self.assertInvalid(EmailSchema({}), {'field-errors': ['email']})
        self.assertInvalid(EmailSchema({'email': ''}),
                           {'field-errors': ['email']})

        self.assertValid(OptionalEmailSchema({'email': 'test@example.com'}),
                         {'email': 'test@example.com'})
        ...
        self.assertInvalid(OptionalEmailSchema({'email': 'test@example'}),
                           {'field-errors': ['email']})
        self.assertInvalid(OptionalEmailSchema({'email': 'test.example.com'}),
                           {'field-errors': ['email']})
    ...
    def test_integer(self):
        ...
        self.assertInvalid(OptionalAgeSchema({'age': ''}),
                           {'field-errors': ['age']})
        self.assertInvalid(OptionalAgeSchema({'age': 0.5}),
                           {'field-errors': ['age']})
    ...
    def test_list(self):
        ...
        self.assertValid(TagsSchema({'tags': ['python', 'ruby']}),
                         {'tags': ['python', 'ruby']})
        self.assertInvalid(TagsSchema({'tags': []}),
                           {'field-errors': ['tags']})
        self.assertInvalid(TagsSchema({'tags': None}),
                           {'field-errors': ['tags']})
        self.assertInvalid(TagsSchema({}), {'field-errors': ['tags']})

        self.assertValid(OptionalTagsSchema({'tags': ['python', 'ruby']}),
                         {'tags': ['python', 'ruby']})
    ...
    def test_choice(self):
        ...
        self.assertValid(ChoiceSchema({'choice': 'Hello'}),
                         {'choice': 'Hello'})
        self.assertInvalid(ChoiceSchema({'choice': 'World'}),
                           {'field-errors': ['choice']})
        self.assertInvalid(ChoiceSchema({'choice': 'invalid'}),
                           {'field-errors': ['choice']})

        class CaseInsensitiveChoiceSchema(Schema):
            choice = Choices(choices=['Hello', 'world'], case_insensitive=True)

        self.assertValid(CaseInsensitiveChoiceSchema({'choice': 'Hello'}),
                         {'choice': 'Hello'})
        self.assertValid(CaseInsensitiveChoiceSchema({'choice': 'hello'}),
                         {'choice': 'Hello'})
        self.assertValid(CaseInsensitiveChoiceSchema({'choice': 'wOrLd'}),
                         {'choice': 'world'})
        self.assertInvalid(CaseInsensitiveChoiceSchema({'choice': 'world '}),
                           {'field-errors': ['choice']})
        self.assertInvalid(CaseInsensitiveChoiceSchema({'choice': 'invalid'}),
                           {'field-errors': ['choice']})
    ...
    def test_url(self):
        ...
        self.assertValid(URLSchema({'url': 'http://example.com/a?b=c'}),
                         {'url': 'http://example.com/a?b=c'})
        self.assertValid(URLSchema({'url': 'ftp://ftp.example.com'}),
                         {'url': 'ftp://ftp.example.com'})
        self.assertInvalid(URLSchema({'url': 'www.example.com'}),
                           {'field-errors': ['url']})
        self.assertInvalid(URLSchema({'url': 'invalid'}),
                           {'field-errors': ['url']})

        class DefaultURLSchema(Schema):
            url = URL(default_scheme='http://')

        self.assertValid(DefaultURLSchema({'url': 'http://example.com/a?b=c'}),
                         {'url': 'http://example.com/a?b=c'})
        self.assertValid(DefaultURLSchema({'url': 'ftp://ftp.example.com'}),
                         {'url': 'ftp://ftp.example.com'})
        self.assertValid(DefaultURLSchema({'url': 'www.example.com'}),
                         {'url': 'http://www.example.com'})
        self.assertInvalid(DefaultURLSchema({'url': 'invalid'}),
                           {'field-errors': ['url']})
        self.assertInvalid(DefaultURLSchema({'url': True}),
                           {'field-errors': ['url']})
