...
def compare_dict_keys(data, keys, test_true=True):
    ...
    for k, v in data.items():
        if test_true:
            assert not v or k in keys, \
                'Key %r is unexpectedly true in %r' % (k, data)
        else:
...
def compare_req_resp(req_obj, resp_obj):
    for k, v in req_obj.items():
        assert k in resp_obj.keys(), \
            'Key %r not in response (keys are %r)' % (k, resp_obj.keys())
        assert resp_obj[k] == v, \
            'Value for key %r should be %r but is %r' % (k, v, resp_obj[k])
